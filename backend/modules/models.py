...
class ServerMetricsModel(BaseModel):
    ...
    cpu_usage: float = Field(
        default=0.0, ge=0, le=100, description="CPU usage percentage"
    )
    memory_usage: float = Field(default=0.0, ge=0, description="Memory usage in MB")
    tps: Optional[float] = Field(
        default=None, ge=0, le=20, description="Ticks per second"
    )
...
class PlayerDataModel(BaseModel):
    """Represents player data stored in NBT format."""

    uuid: str = Field(..., description="Player UUID")
    username: str = Field(..., description="Player username")
    data: Dict[str, Any] = Field(
        default_factory=dict, description="NBT data as a dictionary"
    )
    timestamp: Optional[str] = None
    event_type: Optional[str] = None
    account_data: "PlayerDetails" = Field(
        ..., description="Additional player details from PlayerDB"
    )
...
class ServerConfigModel(BaseModel):
    ...
    # Resource allocation
    min_ram: int = Field(
        default=1024, ge=512, le=131072, description="Minimum RAM in MB"
    )
    max_ram: int = Field(
        default=2048, ge=512, le=131072, description="Maximum RAM in MB"
    )
    ...
    all_players: List[PlayerDataModel] = Field(
        default_factory=list, description="All player data"
    )
    addons: List[AddonModel] = Field(
        default_factory=list, description="Installed addons"
    )
    eula_accepted: bool = Field(
        default=False, description="Whether EULA has been accepted"
    )

    # Playit.gg integration
    use_playit: Optional[bool] = Field(
        default=True, description="Whether this server uses Playit.gg for tunneling"
    )
    playit_secret: Optional[SecretStr] = Field(
        default=None, description="The secret key for playit.gg tunneling"
    )
...
class ServerResponse(BaseModel):
    ...
    status: ServerStatus = Field(
        default=ServerStatus.OFFLINE, description="Current status"
    )
    ...
    metrics: Optional[ServerMetricsModel] = Field(
        default=None, description="Latest server performance metrics"
    )
...
class ServerImportRequest(BaseModel):
    ...
    detect_type: bool = Field(
        default=True, description="Auto-detect server type from contents"
    )
